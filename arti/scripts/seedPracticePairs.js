...
// 1) Adjust this path to wherever your service-account JSON actually lives
...
(async () => {
  ...
  seed.forEach(pair => {
    const ref = db.collection("practice_pairs").doc();
    batch.set(ref, pair);
  });
  await batch.commit();
  console.log("✅ practice_pairs seeded successfully");
  process.exit(0);
})().catch(err => {
  console.error("❌ Error seeding practice_pairs:", err);
  process.exit(1);
});
